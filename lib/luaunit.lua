...
if not write then write = print end
local M={}
local _G = _G
if _ENV then
  _G = _ENV
  io.stdout = {write = function(_,s) write(s) end}
end
...
local function __genSortedIndex( t )
    ...
    for key,_ in pairs(t) do
        ...
        elseif type(key) == 'thread' then
            table.insert( sortedIndexStr, tostring(key))
        else
...
local LuaUnit_MT = { __index = M.LuaUnit }
    ...
    function M.LuaUnit.collectTests()
        ...
        for k, v in pairs(_G) do
            ...
            --bootstrap the setup_each function
            if k == "setup_each" then
              M.LuaUnit.setup_each = v
            elseif k == "tearDown_each" then
              M.LuaUnit.tearDown_each = v
            end
        end
...
--        self.result.startIsodate = os.date('%Y-%m-%dT%H:%M:%S')
    ...
    function M.LuaUnit:protectedCall(classInstance, methodInstance, prettyFuncName)
        ...
        local function err_handler(e)
          ...
          if debug then
            ...
            if textutils then
              return textutils.serialise(err)
            else
              return err
            end
        end
    ...
    function M.LuaUnit:execOneFunction(className, methodName, classInstance, methodInstance)
        ...
        if self.asFunction(self.setup_each) then
          self:addStatus(self:protectedCall(classInstance,self.setup_each,className..'.setup'))
        end
        ...
        if self.asFunction(self.tearDown_each) then
          self:addStatus(self:protectedCall(classInstance,self.tearDown_each,className..'.tearDown'))
        end
    ...
    function M.LuaUnit.expandOneClass( result, className, classInstance )
        ...
        for methodName, methodInstance in sortedPairs(classInstance) do
            ...
            --bootstrap a setup_each function
            if M.LuaUnit.asFunction(methodInstance) and "setup_each" == methodName then
                M.LuaUnit.setup_each = methodInstance
            elseif M.LuaUnit.asFunction(methodInstance) and "tearDown_each" == methodName then
                M.LuaUnit.tearDown_each = methodInstance
            end
        end
    end
